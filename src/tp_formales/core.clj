...
(defn controlar-aridad [lista n]
  ...
  (cond
    (= (count lista) n) n
    (> (count lista) n) (list '*error* 'too-many-args)
...
(defn evaluar-lista-params [lista global local]
  (let [valorLocal (buscar (first lista) local)
        valorGlobal (buscar (first lista) global)]
    (cond
      (seq? (first lista)) (list (first (evaluar (first lista) global local)) global)
      (number? (first lista)) (list (first lista) global)
      (not (error? valorLocal)) (list valorLocal global)
      (not (error? valorGlobal)) (list valorGlobal global)
      :else (evaluar-lista-params (rest lista) global local))))

(defn evaluar-if-aux [condicion evSiVerdadero evSiFalso global local]
